...
формулой нахождения точки пополнения заказа и формулой размещения заказа.
Фреймворк предоставляет предопределенные формулы, математические и логические операторы, а также константы и переменные.
...
- `ROUND(<Number>, [<Number>])` - округляет число до целого или до указанного количества знаков (если передан второй аргумент)
...
```logic
balance <= (S/COUNT(Consumption))*Delay + Delivery*(S/COUNT(Consumption)) AND out == 0
```
...
```logic
(COUNT(Consumption) * SQRT((2 * A * S)/I)) / S AND out == 0
```
...
<table class="table table-bordered text-light">
  ...
  <tr>
    <th>Математические операторы</th>
    <td>&#9989;</td>
    <td>&#9989;</td>
    <td>&#9989;</td>
  </tr>
  <tr>
    <th>Логические операторы</th>
    <td>&#9989;</td>
    <td>&#9989;</td>
    <td>&#10060;</td>
  </tr>
